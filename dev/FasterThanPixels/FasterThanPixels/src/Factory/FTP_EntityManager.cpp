...
void FTP_EntityManager::EntityEndWaiting(Engine::Entity* entity, Engine::EntityState entitystate)
{
	...
	case Engine::EntityState::Add:
		if (entity->E_Tag.compare("Asteroid"))
		{
			...
			if (AsteroidRender)
				Engine::GeneratePhysicBody(this, entity->E_Id, AsteroidRender->Sprite.getPosition(), sf::Vector2f(AsteroidRender->Sprite.getGlobalBounds().width / 2, AsteroidRender->Sprite.getGlobalBounds().height / 2), b2_staticBody, GameManager->G_SceneManager->GetCurrentScene()->SCALE, GameManager->G_SceneManager->GetCurrentScene()->World);
		}
		else if (entity->E_Tag.compare("Planet"))
		{
			...
			if (CheckAnimatation)
				Engine::GeneratePhysicBody(this, entity->E_Id, CheckAnimatation->AnimatedSprite.FrameToDraw().getPosition(), sf::Vector2f(CheckAnimatation->AnimatedSprite.FrameToDraw().getGlobalBounds().width / 2, CheckAnimatation->AnimatedSprite.FrameToDraw().getGlobalBounds().height / 2), b2_staticBody, GameManager->G_SceneManager->GetCurrentScene()->SCALE, GameManager->G_SceneManager->GetCurrentScene()->World);
		}
